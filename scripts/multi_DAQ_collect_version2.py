...
# handles data file writing
from async_daq_data_handler3 import AsyncDAQDataHandler
...
def main(args, master_dir, slave_dir):
    ...
    with DaqDevice(daq_device_params['SLAVE']) as daq_device['SLAVE']:
        ...
        with DaqDevice(daq_device_params['MASTER']) as daq_device['MASTER']:
            ...
            # .. when it starts it will trigger the 'SLAVE'
            #
            #   In CONTINUOUS scan mode the start time for MASTER and SLAVE
            #   .. are the same. get the start time for the master and use that as the start time for 
            #   .. both AsyncDAQDataHandler-s because the slave waits until the master starts
            #
            pre_call = time.time()
            ...
            start_time_epoch = (pre_call + post_call) / 2
            # setup MASTER and SLAVE async_writers
            ...
            # start AsyncDAQDataHandlers using master start time for both
            async_writer_MASTER.begin(start_time_epoch=start_time_epoch)
            async_writer_SLAVE.begin(start_time_epoch=start_time_epoch)
            ...
            try:
                ...
                while True:
                    ...
                    try:
                        ...
                        if args.test and time.time() - start_test >= 10:
                            # breakout!
